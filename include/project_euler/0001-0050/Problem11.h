...
///
/// \file
/// \brief Problem 11: Largest product in a grid
///
//===----------------------------------------------------------------------===//
#ifndef PROJECT_EULER_PROBLEM11_H
#define PROJECT_EULER_PROBLEM11_H
...
public:
  Problem11() : product(0), solved(false) {}
  ~Problem11() = default;
  std::string answer();
  ...
  unsigned long
  bruteForce(const std::array<std::array<unsigned, C>, R> grid) const;
...
private:
  ...
  /// The provided grid for the problem
  const std::array<std::array<unsigned, 20>, 20> problemGrid = {
      {{{8, 2, 22, 97, 38, 15, 0, 40, 0, 75, 4, 5, 7, 78, 52, 12, 50, 77, 91,
       ...
       {{22, 31, 16, 71, 51, 67, 63, 89, 41, 92, 36, 54, 22, 40, 40, 28, 66, 33,
         13, 80}},
       {{24, 47, 32, 60, 99, 03, 45, 2, 44, 75, 33, 53, 78, 36, 84, 20, 35, 17,
         12, 50}},
       ...
       {{19, 80, 81, 68, 5, 94, 47, 69, 28, 73, 92, 13, 86, 52, 17, 77, 4, 89,
         55, 40}},
       {{04, 52, 8, 83, 97, 35, 99, 16, 7, 97, 57, 32, 16, 26, 26, 79, 33, 27,
         98, 66}},
...
template <unsigned long R, unsigned long C>
unsigned long
Problem11::bruteForce(const std::array<std::array<unsigned, C>, R> grid) const {
  auto max = 0ul;
