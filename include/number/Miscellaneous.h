...
template <bool UseSpacesAndHyphens, bool UseBritishGrammar>
struct toEnglishStringImpl<mpz_class, UseSpacesAndHyphens, UseBritishGrammar> {
...
template <typename T, bool UseSpacesAndHyphens, bool UseBritishGrammar>
std::string toEnglishString(const T number) {
  return toEnglishStringImpl<T, UseSpacesAndHyphens,
                             UseBritishGrammar>::convert(number);
}
}
/// \endcond
#endif
