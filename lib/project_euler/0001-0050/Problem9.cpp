...
string project_euler::Problem9::answer() {
  if (!solved)
    solve();
...
std::string project_euler::Problem9::description() const {
  return "Problem 9: Special Pythagorean triplet";
}

void project_euler::Problem9::solve() {
  product = bruteForce(1000);
  solved = true;
}

unsigned project_euler::Problem9::bruteForce(const unsigned sum) {
  // WLOG, assume a < b
...
unsigned project_euler::Problem9::faster(const unsigned sum) {
